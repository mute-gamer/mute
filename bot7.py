...
async def get_community_posts():
    ...
    print("[INFO] Checking community posts...")
    if response.status_code == 200:
        ...
        if post_id_match:
            ...
            if post_id != last_post_id:
                ...
                print(f"[INFO] New post detected: {post_url}")
...
async def get_latest_video():
    ...
    print("[INFO] Checking latest video...")
    try:
        ...
        print("[INFO] API Response:", response)

        if "items" in response and len(response["items"]) > 0:
            video = response["items"][0]
            video_id = video["id"].get("videoId")
            url = f"https://www.youtube.com/watch?v={video_id}"
            print(f"[INFO] Latest video found: {url}")

            if video_id and video_id != last_video_id:
                last_video_id = video_id
                print("[INFO] Sending video notification...")
            ...
            print("[WARNING] No videos found in API response.")
    except Exception as e:
...
async def send_promotion():
    ...
    print("[INFO] Sending promotion message...")
